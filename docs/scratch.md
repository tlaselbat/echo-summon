...
- **Capture**
    - Player right-clicks an allowed mount with `echo_summon:harness_summon_tool` (`HarnessSummonToolItem`) while sneaking.
...
- **Mount Harness** (`echo_summon:mount_harness`)
    ...
    - Links back to its parent summon tool via `echo_summon:harness_summon_tool_item_id`.
...
- **Harness Summon Tool** (`HarnessSummonToolItem` in `src/main/java/com/tabletmc/echo_summon/item/custom/HarnessSummonToolItem.java`)
    ...
    - Server validation/processing (`ServerNetworking.init()`):
        - Verifies the tool in hand is the summon tool; checks allowed entity tag; rejects capturing entities already tagged with `echo_summon:harness_summon_tool_summoned`.
        ...
        - On summon:
            ...
            - Applies the command tag `echo_summon:harness_summon_tool_summoned` and equips a `mount_harness` reconstructed from `mount_harness_data` (re-applies next tick if needed).
            ...
            - Applies cooldown.
        - On dismiss:
            - Works only if the player is currently riding and the mount has the `echo_summon:harness_summon_tool_summoned` tag (i.e., it was summoned by the tool).
        ...
        - On release:
            ...
            - Clears `echo_summon:stored_mount`, removes `CustomModelData`, deletes any inventory `mount_harness` items linked to the tool’s `harness_summon_tool_item_id`, then applies cooldown.
    - Data components and keys:
        - `DataComponentTypes.CUSTOM_DATA` → `echo_summon:stored_mount` (entity NBT), `harness_summon_tool_item_id` (stable UUID-like string), and nested `mount_harness_data`.
...
- **Mount Harness** (`MountHarnessItem` in `src/main/java/com/tabletmc/echo_summon/item/custom/MountHarnessItem.java`)
    - Use on entity (server-side):
        ...
        - For harness-slot entities, toggles `HarnessableMountImpl.echo_summon$setHarnessd(true)` after equip.
    - Equipment assets:
        - Uses registry keys under `assets/echo_summon/equipment/...` to render harness layers per supported entities (e.g., horse, donkey, mule, camel, skeleton/zombie horse).
...
- **Server Handler** (`ServerNetworking.init()`)
    ...
    - Ensures summoned mounts carry the `echo_summon:harness_summon_tool_summoned` command tag for cleanup logic.
- **Client Hooks**
...
- **Harness Tag**
    - `data/minecraft/tags/items/harnesses.json` includes `echo_summon:mount_harness` so vanilla logic treats it like a harness.

## Asset & Rendering Notes

- Item asset pipeline follows the 1.21+ flow documented in `docs/ItemAssets-1.21.md`.
- Mount harnesses leverage equipment asset JSONs under `assets/echo_summon/equipment/` to swap mount body/harness layers.
